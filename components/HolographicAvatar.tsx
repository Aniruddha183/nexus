...
import { Sphere, MeshDistortMaterial, OrbitControls, Grid, Environment } from '@react-three/drei';
...
const CoreMesh: React.FC<{ analyser?: AnalyserNode; isSpeaking: boolean }> = ({ analyser, isSpeaking }) => {
  const meshRef = useRef<THREE.Mesh>(null);
  const materialRef = useRef<any>(null);
  const dataArray = useMemo(() => new Uint8Array(analyser?.frequencyBinCount || 128), [analyser]);
        ...
        <group ref={groupRef}>
            ...
            <mesh scale={[3.8, 3.8, 3.8]}>
             ...
             <mesh scale={[2.5, 2.5, 2.5]} rotation={[0.5, 0.5, 0]}>
                ...
                <meshBasicMaterial color="#555555" wireframe transparent opacity={0.1} />
            </mesh>
        </group>
    )
}
...
const HolographicAvatar: React.FC<AvatarProps> = ({ analyser, isSpeaking }) => {
  return (
    <div className="w-full h-full min-h-[400px] relative rounded-lg overflow-hidden bg-[#121212]">
