    ...
    <div className="min-h-screen bg-bg text-txt-main font-sans p-6 md:p-12">
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <div className="flex justify-between items-center border-b border-border pb-6">
           <button onClick={() => router.push('/')} className="flex items-center gap-2 text-sm text-txt-sec hover:text-txt-main">
             <ArrowLeft className="w-4 h-4" /> Exit
           </button>
           <button onClick={() => window.print()} className="flex items-center gap-2 text-sm text-txt-sec hover:text-txt-main">
...
{error && <p className="text-red-500">{error}</p>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
           <div className="md:col-span-2 bg-[#1a1a1a] border border-border p-8 rounded-lg">
              <h3 className="text-xs font-mono text-accent uppercase mb-4">Assessment Summary</h3>
              <p className="text-lg text-txt-main leading-relaxed font-light">{analytics.summary}</p>
              
              <div className="mt-8 flex gap-8">
                 <div>
                    <span className="block text-4xl font-bold">{analytics.score}</span>
                    <span className="text-xs text-txt-sec uppercase">Total Score</span>
                 </div>
                 <div className="w-px bg-border h-12"></div>
                 <div>
                    <span className="block text-4xl font-bold">{analytics.technicalAccuracy}</span>
                    <span className="text-xs text-txt-sec uppercase">Technical</span>
                 </div>
                 <div className="w-px bg-border h-12 hidden sm:block"></div>
                 <div className="hidden sm:block">
                    <span className="block text-4xl font-bold">{analytics.communicationClarity}</span>
                    <span className="text-xs text-txt-sec uppercase">Communication</span>
                 </div>
              </div>
           </div>
              ...
              <div className="w-full h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart cx="50%" cy="50%" outerRadius="70%" data={chartData}>
                    ...
                    <Radar name="Score" dataKey="A" stroke="#E0E0E0" strokeWidth={2} fill="#E0E0E0" fillOpacity={0.1} />
                  </RadarChart>
                </ResponsiveContainer>
              </div>
           </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
           <div>
              <h3 className="text-xs font-mono text-accent uppercase mb-4 pb-2 border-b border-border">Key Strengths</h3>
           ...
           <div>
              <h3 className="text-xs font-mono text-accent uppercase mb-4 pb-2 border-b border-border">Areas for Improvement</h3>
              <ul className="space-y-3">
