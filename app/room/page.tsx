  ...
  const connectToGemini = useCallback(async () => {
    ...
    if (activeSessionRef.current) {
      try {
        ...
        console.warn("Failed to close previous session", e);
      }
      ...
      <div className="min-h-screen bg-bg flex items-center justify-center p-4 text-txt-main">
        <div className="text-center space-y-4">
          ...
          <button
            ...
            className="px-4 py-2 border border-border rounded hover:bg-white/5"
          >
            Retry
          </button>
        </div>
    ...
    <div className="h-screen bg-bg flex flex-col relative overflow-hidden font-sans">
      ...
      <header className="absolute top-0 left-0 w-full z-10 p-6 flex justify-between items-center pointer-events-none">
        <div className="flex items-center gap-2 pointer-events-auto bg-black/50 px-3 py-1 rounded-full border border-border">
          ...
          ></div>
          <span className="text-xs font-mono text-txt-sec">
          ...
          <div className="bg-[#1a1a1a]/90 border border-border rounded-lg p-4 flex flex-col h-full backdrop-blur-sm">
            <div className="flex justify-between items-center mb-3">
              ...
              <button onClick={() => setIsTranscriptVisible(false)}>
                <X className="w-4 h-4 text-txt-sec" />
              </button>
            </div>
            <div
              ref={scrollRef}
            ...
            >
              {transcript.map((item, idx) => (
                ...
                >
                  <span className="text-[10px] text-accent mb-1">
                    {item.role === "model" ? "AI" : "YOU"}
                  </span>
                  <div
                    className={`px-3 py-2 rounded-lg max-w-[90%] ${
                      item.role === "user"
                        ? "bg-border text-txt-main"
                        : "bg-black/40 text-txt-sec border border-border"
                    }`}
                  >
                    {item.text}
                  </div>
              ...
              {streamingUserText && (
                <div className="flex flex-col items-end opacity-50">
                  <div className="px-3 py-2 rounded-lg bg-border/50 text-txt-main border border-border border-dashed">
                    {streamingUserText}
                  </div>
                </div>
              )}
      ...
      <footer className="absolute bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-4 z-30 pointer-events-auto">
        <button
          ...
          className={`p-4 rounded-full border transition-all ${
            isTranscriptVisible
              ? "bg-txt-main text-bg border-txt-main"
              : "bg-transparent text-txt-sec border-border hover:border-accent"
          }`}
        >
