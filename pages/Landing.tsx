...
const Landing: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-bg text-txt-main flex flex-col font-sans">
      {/* Navbar */}
      <nav className="w-full max-w-6xl mx-auto px-6 py-8 flex justify-between items-center border-b border-border">
        <div className="text-xl font-bold tracking-tight text-txt-main">
          NEXUS
        </div>
        <button
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center px-6 py-20">
        ...
        <p className="text-lg md:text-xl text-txt-sec max-w-2xl mb-12 font-light leading-relaxed">
          Real-time voice simulation. Adaptive questioning. Deep analytical
          feedback. No distractions. Just performance.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-24 w-full max-w-5xl text-left">
          {[
            ...
            <div
              key={i}
              className="p-6 border border-border bg-surface/30 rounded-lg hover:border-accent transition-colors"
            >
              <div className="mb-4">{feature.icon}</div>
              <h3 className="text-lg font-medium text-txt-main mb-2">
                {feature.title}
              </h3>
              <p className="text-sm text-txt-sec leading-relaxed">
                {feature.desc}
              </p>
            </div>
          ))}
